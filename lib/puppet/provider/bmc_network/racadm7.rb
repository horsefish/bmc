  ...
  def self.prefetch(resources)
    resources.each_value do |type|
      ...
      type.provider = new(
          ...
          :ipv4_dns_from_dhcp => (idrac_ipv4['DNSFromDHCP'].eql? 'Enabled') ? :true : :false,
          :ipv4_gateway => idrac_ipv4['Gateway'],
          :ipv4_netmask => idrac_ipv4['Netmask'],
          :ip_source => (idrac_ipv4['DHCPEnable'].eql? 'Enabled') ? :dhcp : :static,
          :ipv4_enable => (idrac_ipv4['Enable'].eql? 'Enabled') ? :true : :false,
          :auto_config => (idrac_nic['AutoConfig'].eql? 'Enabled') ? :true : :false,
          :auto_detect => (idrac_nic['AutoDetect'].eql? 'Enabled') ? :true : :false,
          :autoneg => (idrac_nic['Autoneg'].eql? 'Enabled') ? :true : :false,
          :dedicated_nic_scan_time => idrac_nic['DedicatedNICScanTime'],
          :dns_domain_from_dhcp => (idrac_nic['DNSDomainFromDHCP'].eql? ' Enabled') ? :true : :false,
          :dns_domain_name => idrac_nic['DNSDomainName'],
          :dns_domain_name_from_dhcp => (idrac_nic['DNSDomainNameFromDHCP'].eql? 'Enabled') ? :true : :false,
          :dns_bmc_name => idrac_nic['DNSRacName'],
          :dns_register => idrac_nic['DNSRegister'],
          :duplex => idrac_nic['Duplex'],
          :enable => (idrac_nic['Enable'].eql? 'Enabled') ? :true : :false,
          ...
          :vlan_enable => (idrac_nic['VLanEnable'].eql? 'Enabled') ? :true : :false,
  ...
  def flush
    if @property_flush
      ...
      begin
        ...
        add_key_value_pair('iDRAC.IPv4', 'DNSFromDHCP', Racadm::Racadm.bool_to_s(resource[:ipv4_dns_from_dhcp]))
        ...
        add_key_value_pair('iDRAC.NIC', 'Enable', Racadm::Racadm.bool_to_s(resource[:enable]))
        add_key_value_pair('iDRAC.NIC', 'DNSRacName', resource[:dns_bmc_name])
        add_key_value_pair('iDRAC.NIC', 'DNSDomainFromDHCP', Racadm::Racadm.bool_to_s(resource[:dns_domain_from_dhcp]))
        add_key_value_pair('iDRAC.NIC', 'DNSDomainNameFromDHCP', Racadm::Racadm.bool_to_s(resource[:dns_domain_name_from_dhcp]))
        add_key_value_pair('iDRAC.NIC', 'DNSDomainName', resource[:dns_domain_name])
        add_key_value_pair('iDRAC.NIC', 'AutoConfig', Racadm::Racadm.bool_to_s(resource[:auto_config]))
        add_key_value_pair('iDRAC.NIC', 'AutoDetect', Racadm::Racadm.bool_to_s(resource[:auto_detect]))
        add_key_value_pair('iDRAC.NIC', 'Autoneg', Racadm::Racadm.bool_to_s(resource[:autoneg]))
        ...
        add_key_value_pair('iDRAC.NIC', 'VLanEnable', Racadm::Racadm.bool_to_s(resource[:vlan_enable]))
        ...
        @ini_file.store
        Racadm::Racadm.racadm_call(resource, ['set', '-f', file.path])
      ensure
        file.close
        file.unlink
      end
