...
Puppet::Type.type(:bmc_time).provide(:racadm7) do
  ...
  def self.prefetch(resources)
    resources.each do |_key, type|
      ...
      ntp_config_group = Racadm.parse_racadm racadm_out
      ntp_servers = []
      ...
      time = Racadm.parse_racadm racadm_out
      type.provider = new(
        ntp_enable: Racadm.s_to_bool(ntp_config_group['NTPEnable']),
  ...
  def ntp_enable=(value)
    Racadm.racadm_call(
      resource,
      ['set', 'iDRAC.NTPConfigGroup.NTPEnable', Racadm.bool_to_s(value)],
    )
  end
